...
def restore_log(path, local_rank, mpi_size):
    loaded = [
        json.loads(l)
        for l in open(distributed_maybe_download(path, local_rank, mpi_size))
    ]
    try:
        cur_eval_loss = min(
            [z["elbo"] for z in loaded if "type" in z and z["type"] == "eval_loss"]
        )
    except ValueError:
        cur_eval_loss = float("inf")
    starting_epoch = max(
        [z["epoch"] for z in loaded if "type" in z and z["type"] == "train_loss"]
    )
    iterate = max(
        [z["step"] for z in loaded if "type" in z and z["type"] == "train_loss"]
    )
...
def load_vaes(H, logprint):
    ...
    vae = DistributedDataParallel(
        vae, device_ids=[H.local_rank], output_device=H.local_rank
    )
...
def load_opt(H, vae, logprint):
    ...
    if H.restore_optimizer_path:
        optimizer.load_state_dict(
            torch.load(
                distributed_maybe_download(
                    H.restore_optimizer_path, H.local_rank, H.mpi_size
                ),
                map_location="cpu",
            )
        )
    if H.restore_log_path:
        cur_eval_loss, iterate, starting_epoch = restore_log(
            H.restore_log_path, H.local_rank, H.mpi_size
        )
    else:
