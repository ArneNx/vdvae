...
def sample_from_discretized_mix_logistic(l, nr_mix, diffable=False):
    ...
    if diffable:
        sel = F.gumbel_softmax(logit_probs, hard=False, dim=3, tau=1).float()
        # sel = F.softmax(logit_probs, dim=3)
    ...
    x = means + (F.elu(log_scales) + 1) * (torch.log(u) - torch.log(1.0 - u))

    x0 = x[:, :, :, 0]
    x1 = x[:, :, :, 1] + coeffs[:, :, :, 0] * x0
    x2 = x[:, :, :, 2] + coeffs[:, :, :, 1] * x0 + coeffs[:, :, :, 2] * x1
    x_out = torch.stack([x0, x1, x2], dim=1)
    return torch.clamp(x_out, -1.0, 1.0)
...
class DmolNet(nn.Module):
    def __init__(self, H):
        ...
        self.out_conv = get_conv(
            H.width, H.num_mixtures * 10, kernel_size=1, stride=1, padding=0
        )

    def nll(self, px_z, x):
        return discretized_mix_logistic_loss(
            x=x, l=self.forward(px_z), low_bit=self.H.dataset in ["ffhq_256"]
        )
    ...
    def sample(self, px_z, diffable=False):
        im = sample_from_discretized_mix_logistic(
            self.forward(px_z), self.H.num_mixtures, diffable=diffable
        )
        xhat = (im + 1.0) * 127.5
        if diffable:
            xhat = torch.clamp(xhat, 0.0, 255.0)
        else:
