...
def training_step(H, data_input, target, vae, ema_vae, optimizer, iterate):
    ...
    t1 = time.time()
    stats.update(
        skipped_updates=skipped_updates, iter_time=t1 - t0, grad_norm=grad_norm
    )
    return stats
...
def get_sample_for_visualization(data, preprocess_fn, num, dataset):
    for x in DataLoader(data, batch_size=num):
        break
    orig_image = (
        (x[0] * 255.0).to(torch.uint8).permute(0, 2, 3, 1)
...
def train_loop(H, data_train, data_valid, preprocess_fn, vae, ema_vae, logprint):
    ...
    for epoch in range(starting_epoch, H.num_epochs):
        ...
        for x in DataLoader(
            ...
            training_stats = training_step(
                H, data_input, target, vae, ema_vae, optimizer, iterate
            )
            ...
            if (
                iterate % H.iters_per_images == 0
                or (iters_since_starting in early_evals and H.dataset != "ffhq_1024")
                and H.rank == 0
            ):
                write_images(
                    H,
            ...
            if iterate % H.iters_per_ckpt == 0 and H.rank == 0:
                save_model(
                    os.path.join(H.save_dir, f"iter-{iterate}"),
                    vae,
                    ema_vae,
                    optimizer,
                    H,
        ...
        if epoch % H.epochs_per_eval == 0:
            ...
            logprint(
                model=H.desc, type="eval_loss", epoch=epoch, step=iterate, **valid_stats
            )
...
def write_images(H, ema_vae, viz_batch_original, viz_batch_processed, fname, logprint):
    ...
    lv_points = np.floor(
        np.linspace(0, 1, H.num_variables_visualize + 2) * len(zs)
    ).astype(int)[1:-1]
    for i in lv_points:
    ...
    im = (
        ...
        .reshape(
            [
                ...
                mb * viz_batch_processed.shape[2],
                3,
            ]
        )
    )
...
def main():
    ...
    if H.test_eval:
        ...
        train_loop(
            H, data_train, data_valid_or_test, preprocess_fn, vae, ema_vae, logprint
        )
